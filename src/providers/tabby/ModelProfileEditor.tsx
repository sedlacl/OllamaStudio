...
import { api, type ModelTag, type TabbyLoadOptions, type TabbyLoadPresetData } from '../../types/api'
import PresetBar from '../../components/PresetBar'

export interface TabbyModelProfileEditorProps {
  model: ModelTag
  loading: boolean
  error: string | null
...
export type LoadTabbyDialogProps = TabbyModelProfileEditorProps

export default function TabbyModelProfileEditor({
  model,
  loading,
  error,
  currentLoadedId,
  onCancel,
  onLoad
}: TabbyModelProfileEditorProps): JSX.Element {
  const { t } = useI18n()
  ...
  useEffect(() => {
    let cancelled = false
    void api().getModelProfile({ providerId: 'tabby', modelId: model.name })
      .then((profile) => {
        if (cancelled) return
        setForm({
          maxSeqLen: profile.maxSeqLen == null ? '' : String(profile.maxSeqLen),
          cacheSize: profile.cacheSize == null ? '' : String(profile.cacheSize),
          cacheMode: profile.cacheMode ?? 'FP16',
          tensorParallel: profile.tensorParallel ?? true,
          gpuSplitAuto: profile.gpuSplitAuto ?? true,
          gpuSplit: profile.gpuSplit?.join(', ') ?? '',
