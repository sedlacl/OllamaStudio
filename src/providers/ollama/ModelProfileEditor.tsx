...
export interface OllamaModelProfileEditorProps {
  model: ModelTag
...
function configuredContext(serverConfig: AppConfig | null): number | null {
  return numericValue(serverConfig?.providers.ollama.profileDefaults.numCtx)
}
...
function initialTtl(serverConfig: AppConfig | null): string {
  const fromServer = serverConfig?.providers.ollama.profileDefaults.keepAlive?.trim()
...
function profileToForm(profile: ModelLoadOptions): LoadForm {
  return {
    keepInMemory: profile.keepAlive === '-1',
    ttl: profile.keepAlive === '-1' ? '30m' : profile.keepAlive,
    numCtx: profile.numCtx == null ? '' : String(profile.numCtx),
    numBatch: profile.numBatch == null ? '' : String(profile.numBatch),
    numGpu: profile.numGpu == null ? '-1' : String(profile.numGpu),
    numThread: profile.numThread == null ? '' : String(profile.numThread),
    useMmap: profile.useMmap == null ? 'auto' : profile.useMmap ? 'on' : 'off',
    useMlock: profile.useMlock === true,
    ropeBase: profile.ropeFrequencyBase == null ? '' : String(profile.ropeFrequencyBase),
    ropeScale: profile.ropeFrequencyScale == null ? '' : String(profile.ropeFrequencyScale)
  }
}
...
export type LoadModelDialogProps = OllamaModelProfileEditorProps

export default function OllamaModelProfileEditor({
  model,
  modelInfo,
  serverConfig,
  loading,
  ...
  onCancel,
  onLoad
}: OllamaModelProfileEditorProps): JSX.Element {
  ...
  useEffect(() => {
    ...
    void api().getModelProfile({ providerId: 'ollama', modelId: model.name })
      .then((profile) => {
  ...
  }, [model.name, modelInfo, serverConfig])
